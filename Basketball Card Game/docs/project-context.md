...
- curated special card sets such as `glass full art` should be defined through one copyable manual config block that keeps selected player slugs and their image overrides together; do not auto-pick members by rating when the set is meant to be hand-curated
- curated special card sets should generate additive variant cards with their own ids instead of mutating or replacing a player's base card; every non-legend player keeps a normal card, while specials like `glass full art` and `black matter` layer on top as extra collectible variants
- the copyable curated special-set module should support `enabled` toggles per set so a whole custom set can be turned on or off without deleting its config
- the same curated special-set module should support an optional shared set-level `backdrop` image behind player art, so custom sets can use one uploaded cutout background across the whole set while still allowing `backdrop: null` to ignore it
- curated specials generated through that module should always render as full-art card variants rather than standard half-art cards
- the curated special-set module should support optional `showRating` control and a custom `theme` palette so each set can decide whether OVR is shown and can drive the border/overlay color treatment independently of the team colors
- curated specials should use their set label as the visible rarity label/frame text, and they should expose their own filterable rarity ids in the UI instead of inheriting the base `diamond`/`blackmatter` label
- curated specials should also be harder to pull than normal cards through per-set or per-player `pullWeight`, while still living inside the broader eligible pack pool for their underlying rarity/ability
- locked card previews should reuse the same locked visual treatment as the collection/set cards instead of introducing a separate preview-only locked appearance
- pack-earned reward modals should not interrupt the last reveal frame or the immediate post-pack view; queue those celebrations until the user leaves the active pack flow
- single-card packs should end with a clear `Continue` action that exits the pack cleanly instead of forcing the normal multi-card overview flow
- mobile pack helper copy and CTA state must update from pack state immediately: tap-to-reveal before the first flip, swipe-forward while more cards remain, and continue language once the pack is ready to advance
- locked or unowned collection cards should not spend network budget on full player headshots; they should reuse the shared locked-card presentation and lean on the fallback art layer instead
- card browsing should feel warm immediately: use the lighter card-sized NBA headshot variant first, then warm the first visible card assets in the background for players, set detail, and profile card pickers
- avoid rerendering heavy hidden views on every global state update; collection/profile/stats rendering should stay tied to visibility or an actively open modal
- mobile browsing layouts need global bottom safe-space padding so the fixed bottom nav never traps the final row of content behind it
- on mobile, both favorite-team picking and team-filter selection should fit into compact all-teams-on-screen grids with the main action still reachable without awkward extra scrolling
- collection overview logo grids must clip logos inside their own cells so brand marks never overlap neighboring copy
- profile surfaces should treat horizontal overflow as a bug: keep the page width-clamped, left-aligned showcase grids, and hide x-overflow in editor/achievement pickers instead of letting wide content escape
- modal-heavy flows should keep explicit dialog semantics and labels in the HTML layer, and dynamic helper copy such as pack-stage hints, card-preview hints, and progress pills should use polite live announcements where the text changes during interaction
- mobile sheet-style modals should reserve a real footer row in the shell grid so primary actions like `Save Settings` stay inside the viewport while only the modal body scrolls when needed
- in mobile card preview, the close hint must sit clearly below the card with real breathing room; never let the instructional copy overlap or visually touch the preview card itself
- in the mobile player filter flow, team selection should live behind its own dedicated modal trigger instead of occupying the main filter sheet, and the team picker should reuse the compact favorite-team grid language while supporting multi-select
- mobile pack opening should always offer an explicit bottom CTA after a card is revealed: `Next Card` while more cards remain, then `Continue` on the last or only card, so swipe is optional rather than required
- `Prism` is the curated full-art series that replaces the old `Glass Full Art` naming, and legacy `--glass-full-art` save ids should migrate to `--prism`
- curated special sets belong only to `Special Collection` as their own set entries; they should not be counted inside the normal NBA team sets, even though the cards still remain additive variants of their original players in the overall card pool
- modal-heavy flows should behave like real dialogs for keyboard users too: trap `Tab` inside the topmost visible modal, restore focus when the dialog stack closes, and keep nested mobile sheets/pickers above their parent modal in that same focus model
- the document title should track the live app state instead of staying static, with view-specific titles for packs, collections, players, profile, stats, pack opening, and card preview
- profile overflow should be fixed at the layout source, not just hidden after the fact; decorative profile background layers must stay inside the profile surface bounds and wide profile grids should remain width-clamped
- on small-screen pack opening, set-progress and daily-progress chrome should collapse into compact summary chips once there are multiple updates, so the reveal card keeps priority over auxiliary progress UI
- mobile player team-filter picking should favor logo density over padding: use tighter boxes, larger logos, and team abbreviations instead of full names inside the compact chooser
- desktop team filters should not sit permanently expanded as a full wall of logos; the team picker should live inside a show/hide disclosure so the rest of the filter UI stays readable
- Hall of Fame should never appear as a selectable team filter option; that card pool is reached through the `Legends` rarity instead
- rarity-frame text should ride the visual centerline of the card border, including around rounded corners, and mobile animation should trade a little density/frame-rate for smoother motion instead of stuttering
- premium-shell rarity frames like Black Matter, Legends, Prism, and other curated specials need a deeper frame inset than base cards so the rotating label stays centered in the border band through the rounded corners on both desktop and mobile
- mobile preview cinematic mode should be available on owned special cards broadly, not only on legends or black matter, but locked/missing preview cards must never expose cinematic toggles
- locked cards in players, sets, and preview should still show the player art using the shared missing-card grayscale treatment; do not hide the portrait entirely
- curated special overlays must never cover so much of the player art that the portrait becomes hard to read; readability of the player image wins over extra sheen
- curated full-art specials should use the sharper preview-grade player image asset even in grid/set views; the lightweight card headshot variant is too soft for their larger art area and makes owned specials look blurrier than locked grayscale cards
- for curated full-art specials that use NBA headshots, disable responsive downsampling in grid/set views and feed the full-resolution asset directly; browser-picked 260w/520w variants are still too soft once the portrait is cropped large inside the full-art frame
- Sharpshooter should be treated as its own plain full-art special shell instead of inheriting the generic curated-special overlays; keep the manually chosen player art sources, but render them without extra haze, pattern, or shimmer above the portrait so owned cards stay as readable as the locked grayscale versions
- card preview navigation should follow the source context: team-set order inside sets and the current filtered player order inside the Players browser, with the same arrows/swipe behavior re-used in both places
- pack opening should support one primary action across keyboard and buttons: `Space` on desktop and the visible primary button should open packs, reveal the current card, advance to the next card, and continue when the flow is ready
- while a pack is actively opening, holding `Space` on desktop or holding the primary pack-action button on desktop/mobile should fill a progress state and skip the reveal flow into its resolved outcome instead of requiring every card animation
- desktop pack opening should maintain an active reveal-card selection for arrow-key navigation, and repeated `Space` presses should keep moving the flow forward card by card without extra dead-end states
- curated special player-image overrides are optional, not sacred; if a custom art source renders too soft or unclear in-card, prefer the sharper base player image over preserving the override
- desktop pack reveal cards should reserve the same footer/action-row height that the resolved pack overview uses, so the grid does not jump when reveal transitions into overview
- desktop pack overview should account for the full card footprint, including footer actions, by letting the stage scroll instead of clipping the bottom row when multi-row grids exceed the available modal height
- desktop card preview hints like `Click anywhere outside the card to close.` should sit clearly below the card with extra breathing room and must never visually touch the card border
- desktop pack overview should stay non-scrolling inside the reveal-stage itself; when a resolved pack gets dense, tighten the overview card footprint and surrounding chrome instead of introducing an internal stage scrollbar
- the pack reveal primary CTA should visually match `Open New Pack`, so reveal/skip/continue actions read as the same main progression control across desktop and mobile
- while a desktop pack is actively opening, the primary reveal CTA should live in the same centered wide slot as `Open New Pack`; holding it should temporarily relabel the button to `Skip Reveal`, and any empty repeat/meta stack must fully collapse so the CTA never drifts to the side
- desktop opening-pack grids need a slightly tighter footprint than resolved overview grids so two reveal rows fit cleanly without clipping the bottom edge before the modal transitions into pack overview
- collections, players, and preview renders should explicitly resync cached player images after DOM insertion; otherwise recently loaded special cards can stay visually stuck in fallback-art mode even though the browser already has the real image
- mobile pack reveal should prioritize a larger active card with tighter stage/footer spacing on phone widths; do not shrink the reveal card more than necessary just to create empty vertical air, and hold-to-skip on the primary action should resolve in about one second instead of feeling sluggish
- Sharpshooter needs to stay isolated from the generic full-art shell stack: hide shell glow/fallback/backdrop/pseudo layers with high-specificity overrides, keep the player image fully opaque above the internal art core, and keep any readability gradient concentrated near the bottom so the portrait stays unobscured
- selling duplicates from a resolved pack should always happen in place: sold duplicate cards disappear from the current pack overview instead of closing the pack, and if that empties the pack entirely the UI should open a next-step modal with `Go to Packs` and `Open New Pack` actions rather than leaving an empty dead-end
- Sharpshooter should not use a one-off render shell if that shell diverges visually from the other curated specials; prefer routing Sharpshooter through the same `curated-special-card` render path as the other special full-art sets unless there is a clear visual requirement that justifies separate behavior
